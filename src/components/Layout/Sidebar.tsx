...
export const Sidebar = () => {
  return <div className="w-80 bg-gray-50 border-r border-gray-200 flex flex-col">
      {/* Header with logo */}
      <div className="p-4 border-b border-gray-200 flex items-center justify-between">
        <button className="flex items-center space-x-2 px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
          ...
          <span className="text-sm font-medium">New day</span>
          <span className="text-xs text-gray-500 bg-gray-200 px-1 rounded">K</span>
        </button>
        
        <div className="flex items-center space-x-2">
          <svg width="24" height="13" viewBox="0 0 640 347" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4 space-y-2">
          {previousChats.map(chat => <button key={chat.id} className={`w-full flex items-start space-x-3 p-3 rounded-lg text-left transition-colors ${chat.active ? 'bg-gray-200 border border-gray-300' : 'hover:bg-gray-100'}`}>
